...
cat_area_per_vol = 4500  # I made this up, in m-1. 4500 is lowest that "work" for all base
# 1e6 is too high 5e5, discontinuity around co ratio from first ratio to 1.5, 8e5 discontinuity until 1.4
# 9e5 also has discontinuity until 1.4
# 4500
flow_rate = 4.7  # slpm
...
cat_ratios = np.arange(1000,1e6,100)

for cat in cat_ratios:
    try:
        cat_area = cat * rvol
        ...
        out_dir = 'test'
        os.path.exists(out_dir) or os.makedirs(out_dir)
        fig.savefig(out_dir + '/' + str(cat) + 'conv.png', bbox_inches='tight')
        ...
        out_dir = 'test'
        os.path.exists(out_dir) or os.makedirs(out_dir)
        fig.savefig(out_dir + '/' + str(cat) + 'flows.png', bbox_inches='tight')
