...
cat_area_per_vol = 100000  # in m-1, made up
...
def plotflow(a):
    ...
    fig.savefig(out_dir + '/profile.pdf', bbox_inches='tight')
...
def plotZoom(a):
    ...
    out_dir = 'figures'
    os.path.exists(out_dir) or os.makedirs(out_dir)
    fig.savefig(out_dir + '/profileZoom.pdf', bbox_inches='tight')
...
def plotSurf(a):
    ...
    out_dir = 'figures'
    os.path.exists(out_dir) or os.makedirs(out_dir)
    fig.savefig(out_dir + '/surf.pdf', bbox_inches='tight')
        ...
        if verbose is True:
            if not n % 100:
                print('  {0:10f}  {1:10f}  '.format(dist, *gas[
                    'C2H4(11)', 'C4H8(10)'].X * 1000 * 60 * kmole_flow_rate))
                # print(surf.T)
                # print(gas.P)
...
def batch(gas, surf, temp, mol_in, verbose=False):
    ...
    rxn_time = np.logspace(-4, 5, 1000001) #from 0s to 100s, log spacing\n",
